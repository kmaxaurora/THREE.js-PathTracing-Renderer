...
function init() {
  ...
  function handleCameraProjectionChange() {
    if (!currentlyUsingOrthographicCamera) changeToOrthographicCamera = true;
    else if (currentlyUsingOrthographicCamera) changeToPerspectiveCamera = true;
    // toggle boolean flag
  ...
  pixel_ResolutionController = gui
    .add(pixel_ResolutionObject, 'pixel_Resolution', 0.5, 1.0, 0.05)
    .onChange(handlePixelResolutionChange);
  if (!mouseControl)
    orthographicCamera_ToggleController = gui
      .add(orthographicCamera_ToggleObject, 'Orthographic_Camera', false)
      .onChange(handleCameraProjectionChange);
...
function animate() {
  ...
  if (useGenericInput) {
    if (!isPaused) {
      ...
      if (keyPressed('KeyQ') && !keyPressed('KeyZ')) {
        cameraControlsObject.position.add(
          cameraUpVector.multiplyScalar(cameraFlightSpeed * frameTime),
        );
        cameraIsMoving = true;
      }
      ...
      if (
        (keyPressed('ArrowUp') || button5Pressed) &&
        !(keyPressed('ArrowDown') || button6Pressed)
      ) {
      ...
      if (!keyPressed('KeyO')) canPress_O = true;

      if (keyPressed('KeyP') && canPress_P) {
        changeToPerspectiveCamera = true;
        canPress_P = false;
      }
      if (!keyPressed('KeyP')) canPress_P = true;
    } // end if (!isPaused)
  ...
  if (increaseFOV) {
    worldCamera.fov++;
    if (worldCamera.fov > 179) worldCamera.fov = 179;
    fovScale = worldCamera.fov * 0.5 * (Math.PI / 180.0);
    pathTracingUniforms.uVLen.value = Math.tan(fovScale);
    pathTracingUniforms.uULen.value = pathTracingUniforms.uVLen.value * worldCamera.aspect;
  ...
  if (increaseAperture) {
    ...
    if (apertureSize > 10000.0) apertureSize = 10000.0;
